...
imageCdn = environment.img;
  ...
  left(event) : void {
    ...
    const nextRightElement = this.elref.nativeElement.querySelector('.tjeneste-' + nextRgiht);
    //Update buttons
    this.updateButtons(nextActiveElement.getAttribute("name"));
    //Previous active - Next RIGHT
  ...
  removeClasses(elem) : void {
    elem.classList.remove("l-left", "r-left", "r-right" ,"l-right", "l-active", "r-active");
  }
  ...
  buttonActive;
  ngAfterContentInit() {
  this.buttonDesign = this.elref.nativeElement.querySelector(".btn.tjenester.design"); 
  this.buttonUtvikling = this.elref.nativeElement.querySelector(".btn.tjenester.utvikling");
  this.buttonSeo = this.elref.nativeElement.querySelector(".btn.tjenester.seo");
}

updateButtons(elementName) : void{
  this.buttonActive = this.elref.nativeElement.querySelector(".btn.tjenester.active");
  this.buttonActive.classList.remove("active");
  this.buttonActive = this.elref.nativeElement.querySelector(".btn.tjenester." + elementName);
